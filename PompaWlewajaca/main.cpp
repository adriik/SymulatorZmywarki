...
#define STEROWANIE 6
#define SENSOR 11
#define NAZWA_LOGU "logi/PompaWlewajaca_log"
...
int main(int argc, char *argv[])
{
    ...
    wartosc = doc.child("Zmywarka").child("PompaWlewajaca").child("PompaWlewajacaWartosc").attribute("Wartosc").as_int();
    ...
    while(1)
    {
        ...
        //printf("Otrzymana wartosc sterowania: %d\n", sterowanie);

        //snprintf(buforDlaLogu, sizeof(buforDlaLogu), "Otrzymana wartosc sterowania: %d", sterowanie);
        ...
        if (sterowanie != stanObecny)
        {
            if (sterowanie == 1)
            {
                printf("Pompa wlewajaca zostala wlaczona\n");
                zapiszLog("Pompa wlewajaca zostala wlaczona");
            }
            else
            {
                printf("Pompa wlewajaca zostala wylaczona\n");
                zapiszLog("Pompa wlewajaca zostala wylaczona");
            }
        ...
        else
        {
            ...
            printf("Dostarczono %d jednostek wody\n",wartosc);

            snprintf(buforDlaLogu, sizeof(buforDlaLogu), "Dostarczono %d jednostek wody", wartosc);
            zapiszLog(buforDlaLogu);
        }
        sleep(sensorWodyGornyKrok);
    }
