...
#define WYSYLANIE_INFORMACJI 3
#define ODBIERANIE_INFORMACJI 12
#define NAZWA_LOGU "logi/CzujnikDolnegoPoziomuWody_log"
...
int main(int argc, char *argv[])
{
    ...
    //printf("POZIOM WODY = %lf\n", poziomWody);

    pugi::xml_document doc;
    pugi::xml_parse_result result = doc.load_file("./config.xml");
    wysokoscSensora = doc.child("Zmywarka").child("SensorDolnegoPoziomuWody").child("WysokoscDolnegoSensoraWody").attribute("Wartosc").as_float();
    //dzielnikWartosci = doc.child("Zmywarka").child("Sterowanie").child("DzielnikWartosci").attribute("Wartosc").as_float();
    //krok = doc.child("Zmywarka").child("SensorDolnegoPoziomuWody").child("SensorWodyDolnyKrok").attribute("Wartosc").as_int();

    //wysokoscSensora *= dzielnikWartosci;

    //printf("wysokoscSensora: %d\n",wysokoscSensora);
    //snprintf(buforDlaLogu, sizeof(buforDlaLogu), "wysokoscSensora: %f", wysokoscSensora);
